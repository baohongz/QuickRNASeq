...
// Baohong improvement:  hack to remove ordinal dimensions with many values or minimal values
...
pc.createAxes = function() {
  ...
  g.append("svg:g")
    ...
    .append("svg:text")
      .attr({
        "text-anchor": "middle",
        "y": function(d,i) { return i%2 == 0 ? -4 : -14 },
...
// Baohong: add code to drop axis to make the plot less crowded
		if (dragging[d] < 10 || dragging[d]+__.margin.left+__.margin.right+10 > __.width) {
			delete __.dimensions[j];
			pc.resize();
			pc.updateAxes();
			pc.svg.selectAll(".dimension").on("click", update_colors);
			pc.render();
		} else {
	        if (i !== j) {
